...
maze_t::write(ostream &os) const {
  ...
  for (int i = 1; i <= matrix_.get_m(); i++) {
    for (int j = 1; j <= matrix_.get_n(); j++)
      switch (matrix_(i, j)) {
        case START_ID: os << START_CHR;
          break;
        case END_ID: os << END_CHR;
          break;
        case WALL_ID: os << WALL_CHR;
          break;
        case PASS_ID: os << PASS_CHR;
          break;
        case PATH_ID: os << PATH_CHR;
          break;
      }
    os << endl;
  }
...
// mustra la solución del laberinto
void
maze_t::road_write(ostream &os) const {
  os << "La solución es esta:" << endl;
  
  for(int i = 1; i <= road_.get_m(); i++) {
    for (int j = 1; j <= road_.get_n(); j++) {
      if (road_.at(i,j) == PATH_ID)
        os << PATH_CHR;
      else
        os << PASS_CHR;
    }
...
maze_t::is_ok_(const int i, const int j) const {
  ...
  if (i > 0 && i <= matrix_.get_m() && j > 0 && j <= matrix_.get_n() && matrix_.at(i, j) != WALL_ID
      && !visited_.at(i, j))
    return true;
}
...
maze_t::solve_(const int i, const int j) {
  ...
  road_.resize(matrix_.get_m(), matrix_.get_n());
  // bucle que recorre los vectores de desplazamientos que contienen las coordenadas de las 4 posibles direcciones
  // empezamos por la primera columna de los vectores y N es el primer valor constante del vector direction_t
  for (int k = 3; k >= N; k--) {
    // ulizamos los vectores desplazamientos para calcular las posibles 4 direciones para el recorrido k
    if (is_ok_(i - i_d[k], j - j_d[k]))
      if (solve_(i - i_d[k], j - j_d[k])) {
        matrix_(i - i_d[k], j - j_d[k]) = PATH_ID;
        road_.at(i - i_d[k], j - j_d[k]) = PATH_ID;
        return true;
      }
  }

  otherRoad_.resize(matrix_.get_m(), matrix_.get_n());
  for (int k = 3; k >= N; k--) {
    if (is_ok_(i - j_d[k], j + i_d[k]))
      if (solve_(i - j_d[k], j + i_d[k])) {
        matrix_(i - j_d[k], j + i_d[k]) = PATH_ID;
        otherRoad_.at(i - j_d[k], j + i_d[k]) = PATH_ID;
        return true;
      }
  }
...
// localizar otra salida al mismo camino, y copiarlo en una nueva estructura
void
maze_t::other_road(ostream &os) const {
  os << "La otra solución es esta:" << endl;
  
  for(int i = 1; i <= otherRoad_.get_m(); i++) {
    for (int j = 1; j <= otherRoad_.get_n(); j++) {
      if (otherRoad_.at(i,j) == PATH_ID)
        os << PATH_CHR;
      else
        os << PASS_CHR;
    }
