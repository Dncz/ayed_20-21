...
template <class T> class queue_l_t {
 ...
 public:
  ...
  bool check(const T letra) const;
  // void swapT(const queue_l_t<T>&) const;
...
// EJERCICO 2: Encontrar una letra
template<class T> bool queue_l_t<T>::check(const T letra) const {
  dll_node_t<T>* aux = l_.get_head();
  bool idk = false;
  while (aux != NULL) {
    if (letra == aux->get_data()) {
      idk = true;
      return idk;
    }
    aux = aux->get_next();
...
// 
// template<class T> void queue_l_t<T>::swapT(const queue_l_t<T> cola) const {
