...
public:
  ...
  // ejercicio 1
	void copy(const queue_v_t<T>& cola);

  // ejercicio2
  bool check(const T& dato) const;
...
template<class T>
void
queue_v_t<T>::copy(const queue_v_t<T>& cola) {
  front_ = 0;
  back_ = -1;
  for (int i = cola.front_; i <= cola.back_; i++)
    push(cola.v_.at(i));
}
...
template<class T>
bool
queue_v_t<T>::check(const T& dato) const {
